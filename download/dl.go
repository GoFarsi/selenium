...
func handleFile(file file) error {
	...
	if rename := file.rename; len(rename) == 2 {
		...
		os.RemoveAll(rename[1]) // Ignore errors.
...
func downloadFile(file file) (err error) {
	...
	if err != nil {
		return fmt.Errorf("errors creating %q: %v", file.name, err)
	}
	defer func() {
		if closeErr := f.Close(); closeErr != nil && err == nil {
			err = fmt.Errorf("errors closing %q: %v", file.name, err)
		}
	}()

	resp, err := http.Get(file.url)
	if err != nil {
		return fmt.Errorf("%s: errors downloading %q: %v", file.name, file.url, err)
	}
